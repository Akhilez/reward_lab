...
def sample_action(
    ...
    if apply_epsilon and epsilon is not None:
        if torch.rand(1) < epsilon:
            ...
            return int(torch.argmax(q_values, 1))
    else:
        q_values = F.softmax(F.normalize(q_values), dim=1)
...
def get_q_loss(q_pred, reward, model, state_next, gamma):
    ...
    q_loss = F.mse_loss(q_pred, target_q.squeeze())
    return q_loss
...
def train():
    ...
    for epoch in range(cfg.epochs):
        ...
        while not done:
            ...
            intrinsic_reward = 10000 * get_intrinsic_reward(state, action, state2, icm_model)

            reward = (cfg.intrinsic_weight * min(intrinsic_reward, 10)) + (cfg.extrinsic_weight * extrinsic_reward)
            ...
            if done:
                ...
                log.mean_summed_rewards = (log.mean_intrinsic_rewards + log.mean_extrinsic_rewards) / 2

                if must_record:
                    log = dict(log)
                    log[f"video_ep{epoch}_reward{reward.item()}"] = wandb.Video(_format_video(video_buffer), fps=4, format="gif")
